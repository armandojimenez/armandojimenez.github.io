...
(function ($) {
    ...
    function initLeadBackground() {
        ...
        var $leadContent = $('#lead-content');
        if (!$container.length) return;

        var icons = ['fa-code', 'fa-terminal', 'fa-laptop', 'fa-mobile', 'fa-database', 'fa-server', 'fa-bug', 'fa-keyboard-o', 'fa-gamepad', 'fa-headphones', 'fa-coffee', 'fa-heart', 'fa-star', 'fa-cloud', 'fa-wifi', 'fa-lock', 'fa-shield', 'fa-html5', 'fa-css3', 'fa-linux', 'fa-apple', 'fa-android', 'fa-windows', 'fa-chrome', 'fa-firefox', 'fa-git', 'fa-github', 'fa-cogs', 'fa-rocket', 'fa-bolt', 'fa-magic', 'fa-microchip', 'fa-usb', 'fa-bluetooth', 'fa-battery-full'];

        // Physics Constants - SUPERCHARGED
        var MOUSE_RADIUS = 400; // Larger radius
        var MOUSE_STRENGTH = 2.5; // Stronger push
        var CONTENT_MOUSE_STRENGTH = 1.0; // Text moves too
        var FRICTION = 0.90; // Less sliding, more control
        var SPRING = 0.03; // Looser spring, more float
        var FLOAT_SPEED = 0.015;
        ...
        function createShapes() {
            ...
            while (iconDeck.length < COLS * ROWS) {
                var batch = [].concat(icons).sort(function () {
            ...
            for (var r = 0; r < ROWS; r++) {
                for (var c = 0; c < COLS; c++) {
                    ...
                    var offsetX = (Math.random() - 0.5) * cellW * 0.8;
                    var offsetY = (Math.random() - 0.5) * cellH * 0.8;
                    ...
                    var iconClass = iconDeck[iconIdx++];
                    var $shape = $('<i class="shape fa ' + iconClass + '"></i>');

                    var rotation = Math.random() * 360;

                    // HIGHER OPACITY for visibility
                    var opacity = 0.2 + Math.random() * 0.3; // 0.2 to 0.5
        ...
        function animate() {
            ...
            shapes.forEach(function (shape) {
                var floatX = Math.sin(frame * FLOAT_SPEED + shape.floatOffset) * 15;
                var floatY = Math.cos(frame * FLOAT_SPEED + shape.floatOffset) * 15;
                var homeX = shape.ox + floatX;
                var homeY = shape.oy + floatY;
                ...
                if (dist < MOUSE_RADIUS) {
                    ...
                    var power = force * force * MOUSE_STRENGTH * 30;
                ...
                // Spring back
                var hdx = homeX - shape.x;
                var hdy = homeY - shape.y;
                shape.vx += hdx * SPRING;
                shape.vy += hdy * SPRING;
